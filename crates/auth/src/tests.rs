...
mod tests {
    ...
    fn test_verify_2fa_request_validation() {
        let valid_request = Verify2FARequest {
            login_session_token: "valid.jwt.token".to_string(),
            code: "123456".to_string(),
        };
        assert!(valid_request.validate().is_ok());

        // Wrong code length
        let invalid_code = Verify2FARequest {
            login_session_token: "valid.jwt.token".to_string(),
    ...
    fn test_invite_user_request_validation() {
        let valid_request = InviteUserRequest {
        ...
        let invalid_email = InviteUserRequest {
    ...
    fn test_create_role_request_validation() {
        ...
        // Empty name
        let empty_name = CreateRoleRequest {
            name: "".to_string(),
            description: Some("Manager role".to_string()),
            permission_ids: vec![Uuid::new_v4()],
        };
    ...
    fn test_tenant_status_serialization() {
        ...
        matches!(deserialized, TenantStatus::Active);
    }
    ...
    fn test_audit_event_type_serialization() {
        ...
        let event_type = AuditEventType::LoginSuccess;
        let json = serde_json::to_string(&event_type).unwrap();
        let deserialized: AuditEventType = serde_json::from_str(&json).unwrap();
        matches!(deserialized, AuditEventType::LoginSuccess);
    }
}